...
8. Run `backfillStudyHubResponses` explicitly from the editor for existing responses. Backfill stamps only missing UUIDs and sends up to 25 eligible rows per run, including blank delivery statuses. Repeat backfill while `remaining` is nonzero; the timer does not continue untouched historical rows. `retryStudyHubAutomatically` selects only `retry:` statuses, while `retryPendingStudyHub` selects `retry:` and `error:` statuses for recovery after correcting failures. Neither retry function selects blank, delivered, or unknown statuses. Read summaries in the execution log / `STUDY_HUB_LAST_RESULT` and per-row results in **NexCore Delivery Status**. There is no custom Sheet menu or `onOpen` requirement.

When upgrading an existing script, keep intake disabled and replace/save `Code.gs` with the current source. Existing triggers use the saved code; no new trigger or web-app deployment is needed. Run the header-only connection check and expect `ready`, `enabled:false`. Before enabling, inspect the delivery-status column: the previous timer may already have attempted historical rows and marked them `retry: ingest_disabled`. The update cannot distinguish those attempts from intended retries. For historical rows deliberately excluded from rollout, preserve the original status by prefixing it with `held: `; keep UUIDs unchanged. Held rows are skipped by retries and backfill. Restore their original status only when their import is explicitly authorized. Do not relabel delivered rows. New form submissions still attempt immediate delivery, and failed transient deliveries enter the retry queue.
