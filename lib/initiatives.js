...
function normalizeOptionalLogo(value, errors) {
  if (!value || typeof value !== "object" || !asText(value.src)) return null;
  const src = normalizeImageUrl(value.src);
  if (!isAllowedUrl(src)) errors.push("logo.src must be a valid HTTP(S) or site-relative URL.");
  return { src };
}
...
function validateInitiative(payload) {
  ...
  const data = {
    ...
    logo: normalizeOptionalLogo(source.logo, errors),
